...
| `make reproduce-binary-separation-all` | ~ 8.5 h     | 100 runs — Classes 1+2+3 at the reported arm sizes + z-test |
...
**Scope of the data in this repository.** `avaliacao/` carries the per-run record of all 144 runs executed for the article: the 40 main-matrix runs and 104 binary-separation runs. The **analysed** sample is 140 of those: the 40-run matrix, plus 20 runs per arm for Class 1 and 15 per arm for Classes 2 and 3. The remaining four are a one-iteration smoke sweep of the Class 2 and 3 arms that ran on 2026-05-19 before the amendment fixing 15 per arm was filed; they are published so the exclusion can be checked, and `make reproduce-binary-separation-all` does not re-create them, since a fresh execution cannot reproduce a run that preceded the amendment. Each run directory holds the per-sample JSONL streams and the per-channel summary JSON. Packet captures are not redistributed: `capture.pcap` is 163 MB across the binary-separation runs alone and is regenerated by re-running the experiment, while the protocol-channel features derived from it are published as `modbus_windows.jsonl` and `modbus.summary.json`.
