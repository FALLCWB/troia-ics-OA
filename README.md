...
**Scope of the data in this repository.** `avaliacao/` carries the per-run record of all 144 runs reported in the article: the 40 main-matrix runs and the 104 binary-separation runs across the three trigger classes (Class 1, 20 per arm; Classes 2 and 3, 16 per arm), together with every aggregated CSV/JSON summary. Each run directory holds the per-sample JSONL streams and the per-channel summary JSON. Packet captures are not redistributed: `capture.pcap` is 163 MB across the binary-separation runs alone and is regenerated by re-running the experiment, while the protocol-channel features derived from it are published as `modbus_windows.jsonl` and `modbus.summary.json`.
